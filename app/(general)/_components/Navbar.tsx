        ...
        <nav className="hidden md:flex md:flex-1 md:items-center md:justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            {isPending ? null : session ? (
              <UserDropdown email={session.user.email} image={session.user.image ?? `https://avatar.vercel.sh/${session?.user.email}`} name={session?.user.name[0] && session.user.name.length > 0 ? session.user.name : session?.user.email.split("@")[0]}/>
